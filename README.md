...
Tied to Pinia Store’s Token Getter: Every request grabs `appStore.getToken`. If the new backend uses a different auth mechanism (e.g. session cookie or a custom header key), we’ll need to update this composable—and maybe the store itself—but components don’t need to know these details.
